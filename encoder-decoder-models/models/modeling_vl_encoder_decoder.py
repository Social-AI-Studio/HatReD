...
class VisionLanguageEncoderDecoderModel(PreTrainedModel):
    r"""
    ...
    def forward(
        ...
        **kwargs,
        ...
        # print("encoder_input_ids:", input_ids.shape)
        # print("attention_mask:", attention_mask.shape)
        # print("attention_mask:", input_ids.shape)
        ...
        # print(self.decoder.config.hidden_size)
        ...
        if encoder_attention_mask is None:
            encoder_attention_mask =  torch.cat([attention_mask, visual_attention_mask], dim=1)

        # temp fix: Huggingface's model.generate() expands encoder_hidden_state but does not expand encoder_attention_masks
        if encoder_attention_mask.shape[0] != encoder_hidden_states.shape[0]:
            expand_size = int(encoder_hidden_states.shape[0] / encoder_attention_mask.shape[0])
            expanded_return_idx = (
                torch.arange(encoder_attention_mask.shape[0]).view(-1, 1).repeat(1, expand_size).view(-1).to(encoder_attention_mask.device)
            )
