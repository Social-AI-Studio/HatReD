...
from fhm_data import get_loader
from utils import load_state_dict, LossMeter, set_global_logging_level
import dist_utils
from pprint import pformat

from fhm_model import VLT5FHM
...
class Trainer(TrainerBase):
    ...
    def train(self):
        # Greedy Decoding
        validation_strat = {
            "greedy": {"do_sample": False, "max_length": 50, "num_beams": 1}
        }
        ...
        if self.verbose:
            ...
            #         project_name = "VLT5_FHM"
            #     elif 'bart' in self.args.backbone:
            #         project_name = "VLBart_FHM"
        ...
        for epoch in range(epochs):
            ...
            # Validation
            valid_results = self.evaluate(self.val_loader, validation_strat["greedy"])

            if self.verbose:
                # valid_score = valid_results['CIDEr']
                valid_score = valid_results['harmonic-mean']
                ...
                valid_results.pop("candidates")
                valid_results.pop("references")
                log_str += pformat(valid_results)
                log_str += "\nEpoch %d: Valid Harmonic Mean %0.4f" % (
                    epoch, valid_score)
                log_str += "\nEpoch %d: Best Harmonic Mean %0.4f\n" % (
    ...
    def test(self):
        ...
        for name, strat in strategies:
            print("evaluating...")
            test_results = self.evaluate(self.test_loader, strat)

            # dump candidates and references
            candidates = test_results.pop('candidates')
            references = test_results.pop('references')
            
            candidate_filepath = os.path.join(self.args.output, f"{name}_fhm_candidates.txt")
            reference_filepath = os.path.join(self.args.output, "fhm_references.txt")

            print(candidate_filepath)
            with open(candidate_filepath, 'w') as f:
            ...
            if self.verbose:
                log_str = f'Test set results ({name})\n'
    ...
    def predict(self, loader, gen_kwargs):
        """
    ...
    def evaluate(self, loader, strategy):
        results = self.predict(loader, strategy)
...
def main_worker(gpu, args):
    ...
    test_loader = get_loader(
        args,
        split=args.valid, mode='test', batch_size=valid_batch_size,
        distributed=args.distributed, gpu=args.gpu,
        workers=1,
        topk=args.valid_topk,
    )
    print('# len val loader:', len(test_loader))

    trainer = Trainer(args, train_loader, test_loader, test_loader, train=True)
